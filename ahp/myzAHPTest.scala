...
// scalac -d classes -classpath classes myzAHPTest.scala
...
object Test {
  ...
  def forDAHP4{
    ...
    val tmpPow = myzLinear.Util.powerMethod(tmpSPCMHarker, 100)
    println(tmpPow._2)
    println(tmpPow._3)
    println("総合評価値")
    println(tmpSPCMHarker.aggregateScore(tmpPow._3))
...
//    println(DAHPUtil.readExcelFile("ttt2.xls"))
  ...
  def forSPCM{
    ...
    val tmpPow = myzLinear.Util.powerMethod(matSPCM, 100)
    println(tmpPow._2)
    println(matSPCM.aggregateScore(tmpPow._3))
...
1.0, 2.0
  ...
  def forSPCM2 {
    ...
    println(tmpPow._3)
    println("総合評価値")
    println(matSPCM.aggregateScore(tmpPow._3,0))

    val tmpPow2 = myzLinear.Util.powerMethod(matSPCM, 100)
    println("固有値: " + tmpPow2._2)
    println("固有ベクトル(絶対的重要度の相対値):\n")
    println(tmpPow2._3)
    println("総合評価値")
    println(matSPCM.aggregateScore(tmpPow2._3,0))
...
1.0/3, 1.0/5, 1.0/5, 1.0))
  ...
  def forMatrix {
    val mat1 = new Matrix(List( ((0,0),0.1), ((1,1),0.2), ((1,2),0.3)))
    val mat2 = new Matrix(List( ((0,2),0.4), ((0,0),0.1), ((1,1),0.2), ((1,2),0.3)))
...
1.0/9, 1.0/7, 1.0/5, 1, 1
    ...
    val powerResult = myzLinear.Util.powerMethod(mat5, 100)
    println("itr = " + powerResult._1)
    println("eigenValue = " + powerResult._2)
    println(powerResult._3)

    println("geomMeanMethod: mat5")
    val geomResult = myzLinear.Util.geomMeanMethod(mat5)
